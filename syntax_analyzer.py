"""
    ...
    @brief Implement of syntax analyzer
...
knuth_analyzing_machine = {
    START:
        [[SIGNAL_PROGRAM, 1, False],
         ['#', True, False]],
    SIGNAL_PROGRAM:
        [[PROGRAM, True, False]],
    PROGRAM:
    ...
    STATEMENTS_LIST:
        [[EMPTY, True, True]],
    DECLARATIONS:
        [[MATH_FUNCTION_DECLARATIONS, True, False]],
    MATH_FUNCTION_DECLARATIONS:
        [[404, 1, 2],
...
def syntax_analyzer(list_lexem):
    stack_index = []
    result = -1
    sub_tree = {}
    second_tree = {}
    ...
    while 1:
        if result == True:
            if isinstance(knuth_analyzing_machine[index][i][1], bool):
                l = len(stack_index) - 1
                ...
                if l in sub_tree:
                    ...
                    if l < last_index:
                        st.append([digit_to_word[(index, i)], sub_tree.pop(last_index)])
                        st2.append([(index, i), second_tree.pop(last_index)])
                    else:
                        st.append(digit_to_word[(index, i)])
                        st2.append((index, i))
                else:
                    if l < last_index:
                        st = [[digit_to_word[(index, i)], sub_tree.pop(last_index)]]
                        st2 = [[(index, i), second_tree.pop(last_index)]]
                    else:
                        st = digit_to_word[(index, i)]
                        st2 = (index, i)
                ...
                i = knuth_analyzing_machine[index][i][1]

        elif result == False:
            if isinstance(knuth_analyzing_machine[index][i][2], bool):
                if knuth_analyzing_machine[index][i][2] == True:
                    result = True
                    l = len(stack_index) - 1
                    if l in sub_tree:
                        st = sub_tree.pop(l)
                ...
                i = knuth_analyzing_machine[index][i][2]
        ...
        if index == IDENTIFIER:
            if list_lexem[lexem] in Tab.identifier_tab:
                lex = list_lexem.pop()
                l = len(stack_index) - 1
                if l in sub_tree:
                    st = sub_tree.pop(l)
                    st2 = second_tree.pop(l)
                    st.append(Tab.identifier_tab[lex])
                    st2.append(lex)
                else:
                    st = [Tab.identifier_tab[lex]]
                    st2 = [lex]
                last_index = l
                sub_tree[l] = st
                second_tree[l] = st2
        ...
        if knuth_analyzing_machine[index][i][0] == '#':
            if knuth_analyzing_machine[index][i][0] == list_lexem[lexem]:
                result = True
            else:
                error(list_lexem[lexem], row)
                result = False
        ...
        if knuth_analyzing_machine[index][i][0] < 15:
            stack_index.append([index, i])
            index = knuth_analyzing_machine[index][i][0]
            i = 0
            continue
